...
##### 游标操作
大多数情况下都会有计算符合某个查询条件的文档的数量或者将查询结果按照某个字段进行排序的需求，这种情况下就可以通过游标操作来实现。  
常用的游标操作方法：
* `cursor.count`: 返回查询到文档的数量
* `cursor.limit`: 限制查询到文档的最大数量
* `cursor.skip`: 返回查询结果跳过指定条数的文档
* `cursor.sort`: 将查询到的文档按照指定字段排序。对应字段属性值设为1为升序排列(ascend),如果为-1则为降序排列(descend)

更加详细的介绍请阅读文档： [`cursor methods`](https://docs.mongodb.com/manual/reference/method/js-cursor/)
// 22.查询所有员工的数量
// 23.查看员工集合中的前5条数据
